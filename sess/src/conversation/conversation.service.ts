...
export class ConversationService {
  constructor(
    ...
    async createConversationWithParticipants(body: ConversationWithPDto): Promise<HttpException | Conversation> {
        const owner = await User.findOne({userid:body.ownerID});
        if(!owner) throw new HttpException("User doesn't exist",HttpStatus.NOT_FOUND);
        const conversation = new Conversation();
        conversation.creator = owner;
        ...
        //participants
        let participantsArray:User[] = [owner];
        for(const id of body.participants){
            const participant = await User.findOne({userid:id});
            if(!participant) throw new HttpException("Participant doesn't exist",HttpStatus.NOT_FOUND);
    ...
    async getConversationByUID(uid: string,fid: string): Promise<any> {
        const user = await User.findOne({userid:uid});
        if(!user) throw new HttpException("User doesn't exist",HttpStatus.NOT_FOUND);
        const friend = await User.findOne({userid:fid});
        if(!friend) throw new HttpException("User doesn't exist",HttpStatus.NOT_FOUND);

        // const conv = await Conversation.query(
        //     `SELECT * FROM user,conversation WHERE (creatorUserid="${uid}" or creatorUserid="${fid}") 
        //     and user.userid=conversation.creatorUserid`
        // )
        const conv:any = await User.createQueryBuilder("user")
        .leftJoinAndSelect("user.conversations","conv")
        .leftJoinAndSelect("conv.participants","participant")
        ...
        for(const el of conv.conversations){
            let count = 0;
            if(el.participants[0].userid===uid || el.participants[0].userid===fid) count+=1;
            if(el.participants[1].userid===uid || el.participants[1].userid===fid) count+=1;
