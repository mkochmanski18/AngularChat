  ...
  return jwt.sign(payload,'WSfSfqjVKA495z5qJ9',{ expiresIn})
    ...
    async register(newUser:RegisterDto):Promise<HttpException|userData>{
            ...
            if(checkemail){
              ...
              throw new HttpException("User name is taken!", HttpStatus.BAD_REQUEST)
            }
            else{
                const user = new User();
      ...
      async changeUserdata(userid:string,data:{username:string,email:string,firstname:string,lastname:string,gender:string}):Promise<HttpException>{
        const user = await User.findOne({userid});
        if(!user)
          throw new HttpException("User doesn't exist!", HttpStatus.NOT_FOUND);
        ...
        if(isEmailTaken && isEmailTaken.userid!==user.userid){
          throw new HttpException("Someone is using email already!", HttpStatus.BAD_REQUEST);
        }
        const isNameTaken = await User.findOne({name:username});
        if(isNameTaken && isNameTaken.userid!==user.userid){
          throw new HttpException("Someone is using name already!", HttpStatus.BAD_REQUEST);
        }
        ...
        user.sex = gender;
        user.save();
        throw new HttpException("User data updated!", HttpStatus.OK);
      }
